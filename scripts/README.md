...
5. Create .json file which describes the currency.  Note: 
...
9. This outputs the actual raw transaction in hex:
...
10. Send the transaction by copying and pasting that hex string (without its quotes) as an argument to ``bitcoind sendrawtransaction``
...
11. The output will be the transaction hash ID.  Check http://blockchain.info to see the status of the transation.  You can also find the transaction via blockchain.info's page for the issuing address.
